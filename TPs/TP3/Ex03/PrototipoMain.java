...
public class PrototipoMain{		
	...
	public static void quickRec(Restaurante[] r, int esq, int dir) {
        	...
        	while (i <= j) {
            		while (++comp > 0 && (r[i].getAvaliacao() < pivo || (r[i].getAvaliacao() == pivo && r[i].getNome().compareTo(pivoNome) < 0)))
                		i++;

            		while (++comp > 0 && (r[j].getAvaliacao() > pivo || (r[j].getAvaliacao() == pivo && r[j].getNome().compareTo(pivoNome) > 0)))
                		j--;
	...
	public static void main(String[] args) throws Exception{
		Scanner sc = new Scanner(System.in);
            	...
            	FileWriter arq = new FileWriter("892151_quick_parcial.txt");
