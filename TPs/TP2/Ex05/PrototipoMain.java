...
public class PrototipoMain{		
		...
		public static void pesquisarNome(Restaurante[] restaurantes, String Nome,int n){
                          for(int i = 0; i < n; i++){
				  comp++;	 
                                  if(restaurantes[i] != null && (restaurantes[i].getNome().compareTo(Nome) == 0)){ //busca sequencial por Nome
		...
		public static void main(String[] args) throws Exception{
			Scanner sc = new Scanner(System.in);
            		...
            		while(linha.compareTo("-1") != 0){
               			...
               			linha = sc.nextLine();//leitura da proxima linha
            		}
			...
			while(nome.compareTo("FIM") != 0){
				//System.out.println("Entrando no while de comparar nome");
				pesquisarNome(r,nome,r_qtd); // pesquisa e printa
				nome = sc.nextLine();// escaneia o prox nome
			}
            		fim = System.nanoTime();
...
//            		for(int i = 0; i < r_qtd; i++)
  //              		System.out.println(r[i].formatar());
            		...
            		FileWriter arq = new FileWriter("892151_sequencial.txt");
            		PrintWriter gravarArq = new PrintWriter(arq);

            		gravarArq.printf("892151\t Comparacoes: %d\t Tempo: %.4f\n", comp, total_t);
